...
import type { Customer, MarketPriceSignal, MarketSignal, Product, SalesRecord } from '../domain/core-data.js';
...
export class InMemoryCoreDataRepository implements CoreDataRepository {
  ...
  private readonly marketSignalRecords = structuredClone(marketSignals);
  private readonly salesRecords: SalesRecord[] = [];
  ...
  marketSignals = async (organisationId: string) => this.marketSignalRecords.filter(record => record.organisationId === organisationId);
  salesHistory = async (organisationId: string) => this.salesRecords.filter(record => record.organisationId === organisationId);
  ...
  async upsertSalesRecord(organisationId: string, input: Omit<SalesRecord,'id'|'organisationId'>): Promise<UpsertOutcome> { const existing=this.salesRecords.find(record=>record.organisationId===organisationId&&record.productId===input.productId&&record.marketId===input.marketId&&record.period===input.period); if(existing){Object.assign(existing,input); return 'updated';} this.salesRecords.push({...input,id:`sales-${input.productId}-${input.marketId}-${input.period}`,organisationId}); return 'created'; }
  async upsertMarketSignal(organisationId: string, input: Omit<MarketSignal,'id'|'organisationId'>): Promise<UpsertOutcome> { const existing=this.marketSignalRecords.find(record=>record.organisationId===organisationId&&record.marketId===input.marketId&&record.productId===input.productId&&record.title===input.title); if(existing){Object.assign(existing,input); return 'updated';} this.marketSignalRecords.push({...input,id:`signal-${randomUUID()}`,organisationId}); return 'created'; }
