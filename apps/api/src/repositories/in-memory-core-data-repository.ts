...
export class InMemoryCoreDataRepository implements CoreDataRepository {
  ...
  customers = async (organisationId: string) => customers.filter(record => record.organisationId === organisationId);
  ...
  marketPrices = async (organisationId: string) => marketPrices.filter(record => record.organisationId === organisationId);
