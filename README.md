...
## AI configuration
`AI_PROVIDER=deterministic` (the default) requires no key and keeps the platform fully functional. Set `AI_PROVIDER=anthropic` with `AI_API_KEY` to enable **live Claude** (model `claude-opus-4-8`, override with `AI_MODEL`) for three things: natural-language recommendation explanations, the **"Ask the plan"** assistant (`POST /api/assistant/ask` — answers questions using only the scenario's optimiser results), and **market-news → structured-signal** extraction (`POST /api/assistant/extract-signals`). The AI boundary receives authorised structured fields only and **never calculates or mutates allocations** — the optimiser owns every number; if a live call fails, explanations fall back to the deterministic provider. Never commit credentials.
...
This repository establishes the runnable vertical slice: enterprise dashboard, secured API, approval/audit workflow, full tenant-oriented persistence model, a real constrained optimiser with diagnostics, deterministic demand forecasting, and end-to-end scenario optimisation with explained recommendations. Phase 2 added tenant-scoped product CRUD plus market, customer, inventory, production, and logistics query/calculation modules over a reproducible 25-customer operational dataset; Phase 3 added forecasting, the typed optimiser client, and the scenario workflow; later increments added the interactive UI, the approve/modify/reject decision workflow, Postgres persistence via Prisma, and CSV data import for products and customers. Remaining increments include Express Swagger integration, Playwright coverage, and more importable entities.
